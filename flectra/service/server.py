    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(flectra, 'phoenix', False):
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="flectra.service.autoreload.watcher")
...
def start(preload=None, stop=False):
    ...
    watcher = None
    if 'reload' in config['dev_mode'] and not flectra.evented:
        if inotify:
